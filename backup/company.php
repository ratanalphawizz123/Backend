...
foreach($filed as $key)
{
    ...
    if(array_key_exists($key, $array))
    {
      ...
      $sql3="SELECT $newfiled From $TableName Left JOIN Language on Company.Company_Language = Language.Language_Code Left JOIN Country ON Company.Country_Code = Country.Country_Code LEFT JOIN CompanyUser On Company.Relationship_Manager_ID = CompanyUser.User_ID LEFT Join Country as Countryaddress on Company.Company_Billing_Address_Country = Countryaddress.Country_Code LEFT JOIN State On Company_Billing_Address_State = State.State_Code where $TableName.REC_status='1'";                          
...
foreach($row3 as $column) {
 $id = $row3[0];
  ...
  ?>
      ...
      <a  href="edit_company.php?ID=<?php echo $id; ?>&deltablename=<?php echo $tablename; ?>&pagename=<?php echo $pagename; ?>&Parameter=<?php echo $Parameter ;?>&idname=<?php echo $idname;?>"><i class="fa fa-edit"></i></a>
        <a href="delete.php?ID=<?php echo $id; ?>&deltablename=<?php echo $tablename; ?>&pagename=<?php echo $pagename; ?>&Parameter=<?php echo $Parameter ;?>&idname=<?php echo $idname;?>"><i class="fa fa-trash"></i></a>
                        ...
                        <a href="add_company.php?deltablename=<?php echo $tablename; ?>&pagename=<?php echo $pagename; ?>&Parameter=<?php echo $Parameter ;?>" class="add_button">+</a>
                    </table>
                  </div>
              </div>
            </div>
